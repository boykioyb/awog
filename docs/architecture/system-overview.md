...
- Cửa sổ phụ cùng load SPA: tray popover (`/tray-popover`), preview/session popout, **trình duyệt nhúng popout** (`/browser` — chrome dùng chung với view Browser của workspace panel, [spec](../features/session-browser-panel.md)), và **desktop pet** (`/pet` — transparent + always-on-top + click-through, [spec](../features/desktop-pet.md)). Tất cả dùng chung preload + `applyNavigationGuards`.
