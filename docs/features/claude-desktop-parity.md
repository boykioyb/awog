...
| 14 | Agent tự hẹn giờ thức dậy | 🟡 | Tool `schedule_wakeup` đã ship (dùng lại store lịch của #13, không dựng hàng đợi thứ hai). **Chỉ nhánh Pi**: `runtime/tools/index.ts` đăng ký nó, còn nhánh Claude SDK không bắc cầu — nên đặt provider của agent là `anthropic` là model mất hẳn tool này. Cùng lớp bất đối xứng với cổng quyền của Tasks |
