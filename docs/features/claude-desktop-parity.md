...
| 35 | Artifact có URL/version/bình luận | ✅ ⚠️ | Khuyến nghị cũ ("**không làm** — trái local-first") **SAI** và đã gỡ: nó giả định AWOG phải tự dựng backend lưu trữ. Không cần — CLI đã có sẵn tool built-in `Artifact` (publish/list/read/update, có URL, có version qua `label`, có bình luận), hosting của Anthropic **dưới tài khoản Claude của chính người dùng**. AWOG chỉ mở cổng, không dựng lại gì. Cổng đó **không phải** `settings.enableArtifact` như doc SDK gợi ý: đo thật (SDK 0.3.260/CLI 2.1.260) cho thấy khoá ấy là **no-op** vì CLI đóng theo ENTRYPOINT (`sdk_default_off`) trước khi hỏi tới settings — lever thật là env `CLAUDE_CODE_ARTIFACT=1`. Bảng đo + hàm CLI đã giải mã nằm trong `runtime/claude-sdk/artifact.ts`. Công tắc tắt dùng lại `disabledTools` sẵn có (nhóm Web trong SessionConfigPopover), **không** thêm setting mới — mà đó là công tắc **per-session**, nên nó chỉ tồn tại trên **đường chat**. ⚠️ **Đường task KHÔNG bật tool này** (`runtime/claude-sdk/invoke.ts` cố ý không mang `ARTIFACT_ENV`): commit đầu bật vô điều kiện với lập luận *"node đã có `Bash` dưới `bypassPermissions` nên publish không mở thêm hạng rủi ro nào"*, lập luận đó **sai** — `tasks/engine.ts` (node `discuss`) khai tường minh `disabledTools: ['Write','Edit','MultiEdit','NotebookEdit','Bash']` với chú thích *"discussion must not touch the repo"*, nên một node được thiết kế để không chạm gì vẫn publish được ra URL công khai; cộng thêm việc task chạy **không người trông** (ADR 0024 D-7) và không có công tắc nào ở tầng đó, mặc định phải là TẮT. Mở lại cho task thì thứ phải thêm là **opt-in per-node kèm chỗ hiển thị ra UI**, không phải một dòng env. ⚠️ **Chỉ có trên nhánh Claude SDK (provider `anthropic`) và KHÔNG bắc cầu được** — thân của tool là dịch vụ hosting của Anthropic, không có phần thân AWOG để dùng chung như 7 tool đã bắc. Đổi agent sang OpenAI/Google/Ollama là **mất hẳn, không có bản vá**. Đây là ca đầu tiên trong repo mà mối nguy [ADR 0058](../decisions/0058-claude-agent-sdk-vs-pi-runtime-revisit.md) (*đổi provider là đổi năng lực agent*) **không sửa được, chỉ nói ra được**. Bề mặt cần infosec để mắt: nội dung file `.html` **rời máy** lên hosting của Anthropic — ngoại lệ có chủ đích với local-first. Mỗi lời gọi có hệ quả (`publish` — kể cả khi thiếu `action` — `upload_asset`/`read_asset`/`delete_asset`, và **mọi hành động lạ**) đi qua cổng quyền và bị **chặn cứng trong plan mode**; chỉ `list`/`read`/`list_types`/`list_assets`/`status`/`watch`/`unwatch` chạy thẳng. Chiều mặc định **đảo** so với `browser_tool`/`dev_server` (liệt kê hành động chỉ-đọc, gate phần còn lại) vì thiếu `action` nghĩa là `publish` và vì danh sách hành động do CLI của Anthropic quyết định, không do repo này |
...
| 37 | Marketplace tìm plugin/skill | ✅ | **Code xong** (`templates/marketplace.ts` + 3 RPC `templates.marketplace*` + store + `TemplateDiscoverDialog`/`TemplateConsentPanel`), và từ 2026-09-08 có sẵn **đúng file cần đẩy**: [docs/marketplace-catalog/](../marketplace-catalog/) — 3 entry THẬT trỏ vào bundle first-party `github.com/boykioyb/awog/tree/main/templates/*` (đã đo HTTP 200, tree repo chưa `truncated` nên `planSingleBundle` chạy được), kèm test nạp chính file đó qua `parseCatalog` + `parseGithubUrl` thật — cần vì entry hỏng bị **LOẠI im lặng**, danh mục sẽ thiếu bundle mà không báo gì. Đã **xuất bản 2026-09-09**: repo public `boykioyb/awog-templates` do người dùng tạo, `catalog.json` đẩy lên gốc `main` (commit `db49a54`). Đo lại đúng URL mà `catalogUrl()` dựng (`raw.githubusercontent.com/boykioyb/awog-templates/main/catalog.json`): HTTP 200, nội dung trùng byte với bản trong repo — tức chính file mà test `parseCatalog` đang nạp |
...
| Mode `execute` từ xa bỏ qua permission park | `electron/src/remote-gateway-policy.ts` | ✅ **Xác minh lại 2026-09-09.** `clampRemoteMode` hạ MỌI mode không cổng (`execute`, `accept-edits`) về `ask` trừ khi công tắc **unattended** đang bật (Settings → Devices, mặc định TẮT), và áp cho **cả** mode phiên đã lưu — một `execute` do desktop đặt không được thành séc trắng cho điện thoại. `clampPersistedMode` là câu hỏi khác và xử lý khác: điện thoại không nâng được mode, nhưng cũng không được **hạ** im lặng mode desktop đã đặt (sửa tiêu đề từ điện thoại mà knock phiên ra khỏi `execute` là một lỗi khác). Chuỗi lạ ⇒ `ask`. Test ở `electron/src/__tests__/remote-gateway-policy.test.ts` |
...
Hai dòng cuối vốn nằm ngoài phạm vi các đợt đầu; cả hai nay đã đóng (dòng `remote-gateway-policy.ts` khởi đầu là **ghi chú do chính repo tự viết trong code**, không phải kết luận của lần audit — rà lại 2026-09-09 thì hàng rào đã có sẵn và có test).
...
| F3 | Medium | `dev_server(stop)` giết tiến trình mà không qua cổng quyền; `send_session_message` nhận `session_id` tuỳ ý (không buộc nằm trong danh bạ vừa liệt) | ✅ **Cả hai nửa, xác minh lại 2026-09-09.** `stop` gate theo hành động (`isMutatingDevServerAction`, wired ở `runtime/permission.ts:143`) — `list`/`start`/`logs` cố ý KHÔNG gate, vì bắt duyệt cả việc đọc log là cách nhanh nhất biến rào chắn thành thứ người dùng tắt đi; test `dev-server-gate.test.ts` khoá **cả hai cách gọi tên** (trần + bắc cầu). Đích của `send_session_message` phải qua `isAddressable` (`sessions/inbox.ts:322`), trả `unreachable-target` kèm lý do; ràng bằng **vị từ** chứ không phải danh sách đã cắt ở `CONTACT_LIMIT` — ràng theo danh sách thì cùng một lời gọi lúc chạy lúc không tuỳ số phiên mở trong 24h |
| F4 | Low | Một source của người dùng tên trùng 10 server dành riêng bị AWOG ghi đè **im lặng** — tool của họ biến mất không cảnh báo | ✅ **Xác minh lại 2026-09-09.** Ba đường, danh sách tên derive từ `runtime/tools/bridged.ts` chứ không chép tay: biên TẠO/SỬA fail-closed (`sources/reserved.ts` qua `source.upsert` + `source-tools`), di trú tự đổi tên thành `<id>-mcp` (`sources/migrate.ts` — ở đó không có ai để hỏi), và config đã nằm sẵn trên đĩa thì `run-stream.ts:616` `log.warn` nêu đích danh id. Cố ý KHÔNG đưa vào `SourceConfigSchema`: schema còn là đường ĐỌC, config cũ bị schema loại thì source biến mất khỏi danh sách — tệ hơn hẳn hiện ra kèm cảnh báo |
