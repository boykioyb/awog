...
| 2 | Browser: a11y tree · console · network · viewport · multi-tab | ⬜ | `browser-tool.ts` chỉ có 5 action; nhánh Claude SDK **không có browser nào cả** ⇒ năng lực duyệt web *giảm* khi sang runtime mạnh hơn |
...
| 6 | Monitor / wait-for-condition | ⬜ | Pi không có. SDK có nhưng AWOG xếp vào `AMBIENT_TASK_TYPES` ⇒ monitor persistent chết theo turn |
| 7a | Subagent chọn model lúc gọi (Pi) | ⬜ | Pi lấy model từ AGENT.md, không chọn được tại call site |
| 7b | Subagent chạy nền (Pi) | ⬜ | `task-tool.ts` là one-shot đồng bộ |
| 7c | Subagent trong worktree cô lập | 🟡 | Đã làm cho **Task node** (ADR 0081). Subagent của session thì chưa |
| 7d | Subagent fork context của cha (Pi) | ⬜ | Pi kế thừa *cấu hình*, không kế thừa *lịch sử* |
| 7e | Nhắn tiếp cho subagent cũ (Pi) | ⬜ | Cần `ListAgents`/`SendMessage`, chưa có |
...
| 17 | Session nhắn cho session | ⬜ | `invitedAgentIds` là agent trong *một* phiên, không phải kênh liên lạc |
