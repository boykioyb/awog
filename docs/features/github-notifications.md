...
**Đóng panel hay không**: chỉ hành động **điều hướng trong app** mới đóng panel (bấm thân dòng → mở drawer PR/Issue phía sau panel). `↗ Mở trên GitHub` (cả ở dòng và ở footer) **giữ panel mở**: app không đổi gì, trình duyệt lấy focus, và người dùng thường mở liên tiếp vài dòng — đóng lại là bắt họ mở panel lại từ đầu mỗi lần.
...
- **Nhóm không theo dõi mang chip `không theo dõi`**, KHÔNG làm mờ. Bản trước dùng `opacity: .68` cho cả nhóm; khi chưa chọn project nào thì *mọi* nhóm đều "không theo dõi" ⇒ cả list bị mờ và chữ khó đọc, mà opacity cũng không phải thứ người dùng giải mã được. Chip nói thẳng bằng chữ.
- **Sàn tương phản cho chữ nhỏ = `--textDim`, KHÔNG `--textFaint`**: ở theme sáng `--textFaint` (#8e8e93) trên nền trắng chỉ ~3.4:1 nên `#số`, `@author`, thời gian, số lượng nhóm, footer đọc rất mệt. Đã nâng hết sang `--textDim` (#6e6e73, ~5.3:1). `--textFaint` chỉ còn cho thứ thuần trang trí.
...
- **Tiêu đề chưa đọc in đậm** (`font-weight: 600`) + màu `--text`; đã đọc về `400` + `--textDim`. Vì đa số dòng là chưa đọc, **độ đậm** mới là thứ tách được "còn cần xử lý" khỏi "đã xong" — màu nền một mình không đủ.
...
- **AC18** — Bấm `↗ Mở trên GitHub` ⇒ mở trình duyệt và **panel vẫn mở**; bấm thân dòng ⇒ panel đóng và drawer trong app mở.
