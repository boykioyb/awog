...
> - **Giới hạn — nói đủ để không ai tưởng đã phủ hết.** Vân tay phủ file `.json` và các script mà `command` gọi. Nó KHÔNG phủ, và không thể phủ, những **chương trình mà script gọi tiếp**. Đây không phải góc hiếm: cả hai hook `format-after-edit.sh` thật trên máy dev đều chạy `pnpm exec prettier` / `ruff`, mà `prettier` đến từ `node_modules` — tức **cũng đi theo repo**. Một repo lạ không cần đụng vào script hook: chỉ cần ship một `node_modules/.bin/prettier` độc.
>
>   Vì sao **không** vá bằng cách quét tĩnh `source`/`.`: một luật như thế bỏ qua đúng dạng đang được dùng (gọi binary), và lách được trong một dòng — `bash -c "$(curl …)"` không phải `source`. Chặn `source` mà thả `curl | sh` là **nghiêm khắc giả**: nó bán một cảm giác an toàn không có thật, đúng thứ [ADR 0080 F14](../decisions/0080-command-scoped-permission-rules.md) gọi là nguy hiểm hơn cả không có tính năng. Cơ chế này chống **"đổi mã dưới chân đồng ý cũ"** — và chỉ thế. Nó không phải sandbox, và màn duyệt nay nói thẳng điều đó.
