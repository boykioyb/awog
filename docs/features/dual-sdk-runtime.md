...
- **Vị trí store**: SDK ghi transcript/resume/compaction vào `<claudeHome>/projects/<cwd-hash>/<uuid>.jsonl`. Từ [ADR 0070](../decisions/0070-share-claude-home-for-config.md) AWOG **không override `CLAUDE_CONFIG_DIR`** nữa (trước đó trỏ `~/.awog/claude-sdk`, khiến `Skill` tool của SDK quét một thư mục không có `skills/`) → store nằm chung `~/.claude/projects/` với session CLI thật của user, và session AWOG sẽ hiện trong `claude --resume`. ⚠️ Đây là **store thứ 2** song song JSONL của AWOG (JSONL = bản ghi UI; SDK store = nguồn context/resume) — fork/regenerate/edit/xoá ở UI cần đồng bộ sang SDK store (dùng `forkSession`/`resumeSessionAt` + cleanup file khi xoá session) — follow-up.
