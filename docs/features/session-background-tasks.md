...
- **P4 — Đánh thức khi KHÔNG CÒN CỬA SỔ. ✅ Done (2026-09-07, [ADR 0084](../decisions/0084-wake-when-no-window-and-push-scope.md)).** P2 để lộ một lỗ: wake do renderer lái nên **đóng cửa sổ UI là hết wake** — engine vẫn chạy (macOS giữ app sống ở Dock/tray), lệnh nền vẫn xong, nhưng không còn ai nghe `session.background-done`; `pendingWakes` sống trong RAM của renderer nên mở lại app cũng không có card. Sửa **hoàn toàn ở Electron main**, sidecar và `stores/sessions.ts` không đổi một dòng:
  - **Park + phát lại nguyên văn** — [electron/src/wake.ts](../../apps/desktop/electron/src/wake.ts) nghe `engine.onEvent`, tính **cửa sổ sở hữu** phiên theo đúng luật hand-off của popout (phiên đã pop ra ⇒ cửa sổ popout sở hữu; còn lại ⇒ cửa sổ chính). Không cửa sổ nào nhận được ⇒ park (trần 50, bỏ cái cũ nhất) ⇒ khi renderer đăng ký nghe lại thì phát lại **nguyên văn** trên chính kênh `engine:event`. Store xử lý y như lúc nhận trực tiếp: không có nhánh code thứ hai. Tín hiệu "renderer sẵn sàng" do preload gửi (`engine:subscribed`); main **chờ 750ms** rồi mới phát vì trong renderer có nhiều nơi cùng gọi `onEvent` và cái đầu tiên không chắc là store phiên.
  - **Thông báo OS phát từ main** — [electron/src/notify.ts](../../apps/desktop/electron/src/notify.ts). Trước đây mọi thông báo đều là Web Notification **của renderer** (`useNativeNotify.ts`) nên chết theo cửa sổ. Ba nguồn, **chỉ khi không cửa sổ nào nhận được sự kiện** (nên không bao giờ trùng thông báo của renderer): `session.background-done` (có park), `session.permission-request`, `session.message.done`. Bấm thông báo → mở cửa sổ + đi tới **đúng phiên** qua kênh `tray:command` sẵn có; cửa sổ chưa sẵn sàng thì đích được để dành và preload kéo về đúng lúc gắn handler (`wake:drainRoute`). Công tắc `whenClosed` (mặc định BẬT) do main giữ ở `userData/notifications.json`, hiện ở Settings → Thông báo — main không đọc được settings của renderer, mà lúc cần nó nhất thì renderer không tồn tại.
  - **KHÔNG tự chạy lượt LLM khi không ai nhìn.** Park xong là dừng; `autoContinueOnBackground` vẫn quyết định như cũ, chỉ khác là chạy lúc cửa sổ mở lại. Muốn headless phải có primitive "bắt đầu một lượt" ở sidecar (không có) và nó tiêu tiền thật khi không ai giám sát.
  - **Giới hạn còn lại:** hàng đợi sống theo vòng đời **app**, không qua lần khởi động lại — shell exit lúc app tắt hẳn vẫn chỉ được phát hiện khi mở phiên ra xem (`reloadBackgroundShells` cố ý coi shell đã exit từ tiến trình trước là "đã đọc"). Windows/Linux đóng cửa sổ cuối là thoát app nên cơ chế này thực tế chỉ ăn ở macOS. **Tasks/Workflows chưa có** wake lẫn thông báo-khi-đóng-cửa-sổ.
  - **PWA Remote:** thêm thông báo cho việc nền xong (trước chỉ có cổng quyền + lượt xong), đi qua `registration.showNotification` của service worker như các thông báo khác. **Không có Web Push** và sẽ không có: nó cần push service của trình duyệt (FCM/autopush/APNs) ⇒ lộ metadata ra bên thứ ba ⇒ đụng invariant #5 và phá mô hình tailnet-only — xem mục 4 của ADR 0084.
...
| Điều kiện đóng | **`session_state_changed: idle` đóng — CHỈ KHI `waitingCount() === 0`.** `result` KHÔNG phải turn-over: nó chỉ arm timer settle 4s (mọi message sau đó huỷ timer) khi không còn task nào phải chờ, còn task sống thì **park** — tiếp tục đọc stream, model được CLI đánh thức và phần trả lời tiếp theo chảy vào **cùng turn đó**. Đang park thì turn kết thúc theo đường thường: notification của task cuối kéo `waitingCount()` về 0 → grace 45s đóng, hoặc cap bắn. Chỉ nhận `idle` **sau** khi đã thấy `result` của turn này: CLI vừa resume phát `session_state_changed: running` trước cả `init`, và một báo cáo idle sớm sẽ đóng stdin ngay đầu turn. Điều kiện `waitingCount()` là bản sửa 2026-09-12 bên dưới — thiếu nó thì `idle` giết đúng việc nền mà `park` vừa hứa chờ |
...
### Bug 2026-09-12 — `idle` giết việc nền mà `park` vừa hứa chờ

Người dùng báo "hay bị background interrupted". Chip `interrupted` (= `exited-unknown`) đến từ `settleAllExternalBackground()` trong `finally` của turn: nó chỉ settle task **còn đang chạy**, tức process CLI đã chết trong khi việc nền còn sống.

**Bằng chứng** (`~/Library/Logs/@awog/desktop/main*.log`, 21 lần park):

| Giai đoạn | Số park | Giữ được bao lâu |
|---|---|---|
| Trước [cb44ef8](https://github.com/boykioyb/awog/commit/cb44ef8) (2026-09-06) | 11 | 38s → 900s (3 lần `turn complete`, 5 lần chạm cap, 3 lần user abort) |
| Sau cb44ef8 | 10 | **1–47 ms, không sót cái nào**, 100% `reason="session idle"` |

Ví dụ nguyên văn: `08:14:08.355 turn parked on background work tasks=1 capMs=900000` → `08:14:08.365 closing input reason="session idle"`.

**Nguyên nhân:** cb44ef8 (sửa "denial ảo" 2026-09-06) chuyển điểm đóng từ `result` sang `idle` nhưng nhánh `idle` **không xét `waitingCount()`**. Cơ chế park phía trên vì thế thành code chết. Giả định "idle = bg-agent loop đã thoát" lấy từ `sdk.d.ts`; đo thật thì `idle` bắn ngay sau `result` kể cả khi một `local_bash` của ta còn chạy — `idle` chỉ có nghĩa **"không đang sinh chữ"**.

**Sửa:** `if (m.state === 'idle' && sawResult && waitingCount() === 0)`; đang park thì ghi **một** dòng `idle while parked — holding the turn open` (đủ để phân biệt park lành mạnh với set task bẩn trong log) và để grace/cap lo phần kết. Kèm đó nâng cap 15 → 30 phút và đưa con số vào turn prompt: cap là **cách chết phổ biến thứ hai** của một watcher (5/11 park trước regression chết vì nó), mà thứ nó theo dõi — build, deploy — chạy quá 15 phút là chuyện thường.
