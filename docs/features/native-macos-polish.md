...
- **Bẫy overlay phủ toàn cửa sổ (sửa 2026-09-08):** modal edge-to-edge (`PreviewModal`, `SessionTurnFullscreen`) phủ luôn `.top`, nên **header của chính nó** rơi xuống dưới nút hệ thống — trên macOS 3 đèn đè lên đường dẫn file, trên Windows/Linux glyph minimise/maximise/close đè lên ✕ của header. Hai dải này giờ được công bố thành token trên `<body>` (`--titlebar-inset-start` / `--titlebar-inset-end`, mặc định `0px`) và header full-bleed cộng vào padding của nó; surface edge-to-edge mới chỉ cần dùng token, không lặp lại số đo. Cổng chọn cửa sổ là `body:has(.app)` — chỉ layout shell render `.app`, còn popout (preview/session/tray/pet) giữ **frame native** nên đèn nằm phía trên nội dung và inset ở đó là khoảng chết; overlay được `Teleport` ra `<body>` (ngoài `.app`) nên cổng buộc phải đặt trên `<body>`, không phải trên tổ tiên của header.
