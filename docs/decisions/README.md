...
| [0074](./0074-session-message-anchor-and-transcript-navigation.md) | Neo tin nhắn bền (`eid` cho mọi role) + hợp đồng mở render-window rồi scroll; lớp data-find tách khỏi DOM-highlight | Accepted (T0a amended by 0075, §Q1 prune list by 0076) |
...
| [0041](./0041-in-house-design-system-shadcn-style.md) | Design system in-house kiểu shadcn trên `useTheme` tokens (`AppButton`/`AppCard`) | **Superseded by 0079** — mô tả code đã xoá cùng `apps/desktop/ui/` |
| [0079](./0079-native-macos-shell-and-design-tokens.md) | Vỏ cửa sổ native (title bar ẩn + drag region) + thang design token cho ui-next (`--r-*`, `--fs-*` bằng `calc(base ± Npx)`, mono chỉ cho code) | Accepted |
| [0080](./0080-command-scoped-permission-rules.md) | Luật quyền gắn với **nội dung lệnh** (`Bash(git status)`), không phải tên tool; 3 tầng session/project/user, DENY thắng ALLOW, lệnh có toán tử shell không luật nào khớp | Accepted |
| [0081](./0081-task-node-worktree-isolation.md) | Node task song song thứ 2+ chạy trong `git worktree` riêng (node đầu giữ cây gốc), merge tại điểm ráo; ngân sách cấp task (USD/tool call/wallclock) | Accepted |
| [0082](./0082-scheduled-runs.md) | Chạy theo lịch (Scheduled Runs): union rời rạc thay cron string, quá hạn chạy bù đúng một lần, luôn có trần chi tiêu | Accepted |
| [0083](./0083-pi-subagent-parity.md) | Ngang bằng khả năng subagent giữa nhánh Pi và nhánh Claude SDK | Accepted (§c còn Proposed) |
| [0084](./0084-wake-when-no-window-and-push-scope.md) | Đánh thức khi không còn cửa sổ + phạm vi "push notification" | Accepted |
| [0085](./0085-workflow-as-script.md) | Workflow định nghĩa bằng script + cache theo hash input | **Rejected** (script) · **Deferred** (cache) |
| [0086](./0086-embedded-browser-panel.md) | Nhúng trình duyệt của agent vào panel session (`WebContentsView`) | Accepted |
| [0087](./0087-codex-app-server-as-openai-runtime.md) | Codex `app-server` làm runtime cho account OpenAI | **Proposed** |

> Lưu ý: số `0032` bị trùng (hai quyết định độc lập landed cùng đợt). Số mới không tái sử dụng — ADR kế tiếp dùng số tăng dần. Các ADR 0042–0058 và 0063–0066 đã tồn tại trong thư mục nhưng bảng này chưa backfill (index đang lạc hậu). ADR 0062 **amend** ADR 0048 (thay `index.json` bằng header per-file + warm cache); ADR 0070 **supersede** phần nhà-lưu-trữ của ADR 0035 (skills/agents/commands chuyển sang `.claude` dùng chung); ADR 0075 **amend** phần T0a/§Q2 của ADR 0074 (query `[data-mi]` trong root của transcript + provide/inject theo surface, thay singleton cấp module); ADR 0076 **amend** danh sách "đường cắt" trong §Q1 của ADR 0074 (`/compact` không cắt transcript ⇒ không prune bookmark; quyết định neo bằng `eid` giữ nguyên); ADR 0077 **amend** ADR 0071 (thêm loại block "đính chính bề mặt" được phép đắp lên preset `claude_code`, và bỏ câu *"Keep all of it legible in a terminal."* trong `COMMUNICATION_PROMPT`); ADR 0078 **amend** bảng mapping thinking của item 6 trong ADR 0029 (nhánh Pi chuyển sang 1:1 toàn thang với picker Claude Code; phần degrade theo `model.reasoning` giữ nguyên); ADR 0079 **supersede** ADR 0041 (lớp primitive `components/ui/` mô tả trong 0041 — và trong [ADR 0044](./0044-adopt-shadcn-vue-real.md) — đã bị xoá cùng `apps/desktop/ui/` khi rebuild sang `ui-next`; hướng hiện hành là design token + guard script, xem thêm [ui-design-system.md](../features/ui-design-system.md) đã gắn banner tài-liệu-chết); ADR 0080 vá lỗ hổng leo thang quyền của cơ chế "always allow" theo tên tool; ADR 0085 **không** supersede gì — nó từ chối một hướng và phát biểu luật đứng D-4 (*tier trong repo không mang mã thực thi, không tự mang trust của chính nó*) áp cho mọi feature sau; ADR 0088 (ngữ cảnh hạ tầng AWS/Terraform/kubectl theo phiên) **không** supersede gì — nó mở rộng khuôn Session↔tài nguyên của ADR 0063/0064 bằng cấp thứ ba `forbidden` (lệnh AWOG không bao giờ chạy) và thêm danh sách lệnh không-thể-allow-trọn-gói lên trên ADR 0080; ADR 0089 (bậc "Ultracode" của Claude Code) **amend** ADR 0078: bảng 1:1 năm bậc giữ nguyên, nhưng nhánh Claude SDK có thêm một bậc NGOÀI thang `EffortLevel` — cờ `Settings.ultracode` (xhigh + điều phối dynamic-workflow) đi ở lớp `settings`, hiện ra là bậc thứ sáu của picker effort trong PHIÊN và chỉ với provider `anthropic`; ADR kế tiếp sau 0089 dùng 0090.
...
- Giá trị trạng thái: **Proposed**, **Accepted**, **Deprecated**, **Superseded by NNNN**. Một quyết định "không làm" ghi **Rejected** (kèm điều kiện mở lại) — nó vẫn là quyết định và vẫn cần ADR.
