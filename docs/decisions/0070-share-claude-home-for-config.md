...
| **D-6** | Xử lý trùng tên khi migrate | **Bản `.claude` LUÔN thắng.** Bản `.awog` chỉ bị xoá khi **byte-identical**; nếu khác thì park vào `~/.awog/migrated-conflicts/<kind>/<id>`. | `.claude` là bản đang sống; nhưng "look before delete" — không bao giờ ghi đè/xoá một bản sửa khác nội dung. |
