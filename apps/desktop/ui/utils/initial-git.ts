...
// Classify a mock ref name into a structured decoration. Local heads are
// `feat/...` / `main` / `develop`; anything prefixed with `origin/` becomes a
// remote tracking branch. Keeps fixtures readable as plain strings above.
const toMockRef = (name: string): GitCommit['refs'][number] => {
  if (name.startsWith('origin/')) return { kind: 'remote-branch', name }
...
const buildCommits = (
  ...
  subjects.map((subj, i) => {
    ...
    return {
      ...
      refs: (branchRefs[i] ?? []).map(toMockRef),
