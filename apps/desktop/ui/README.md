...
✅ **Projects** — CRUD với link existing / clone from git, project detail với session list (+ nút New session điều hướng sang Sessions) và task list, 3-col meta grid. Project detail có **thanh tab** (Overview / Issues / Pull Requests); hai tab GitHub chỉ hiện khi `gitRemote` trỏ GitHub — list full-width + filter state/assignee/search, drawer resize được, dịch LLM theo từng đoạn (title/body/comment) qua `gh.*` RPC ([ADR 0049](../../../docs/decisions/0049-github-issues-and-prs-via-gh-cli.md), [spec](../../../docs/features/project-github.md)). Account GitHub chọn ở cấp app (settings store `githubAccount`).
...
✅ **Session upgrades** — AI auto-title sau turn đầu; **Edit & resend** (sửa user message), **Regenerate** + **Retry-with-model** (agent message) qua truncate primitive; **Rewind** (RotateCcw) khôi phục hội thoại + file workspace từ fs snapshot ([ADR 0038](../../../docs/decisions/0038-session-rewind-fs-snapshots.md)); **Cmd+K** cross-session full-text search (`SessionSearchPalette`). Component: `MessageRetryMenu`, `SessionSearchPalette`; composable `useSessionModels`. Xem [docs/features/session-upgrades.md](../../../docs/features/session-upgrades.md).
...
✅ **NavRail shell (UI renew, [ADR 0052](../../../docs/decisions/0052-navrail-shell-flat-default.md))** — navigation chuyển lại sang **NavRail dọc bên trái** (`NavRail.vue`) thay `HeaderTabBar.vue` (giữ tạm để rollback): nhóm Home / Work / Library / System, thu gọn icon-rail (persist `awog.navrail.collapsed`), + `AppTopBar.vue` (title route + global search ⌘K + What's New + theme toggle + slot action). **Badge sống** port nguyên: Sessions unread/awaiting/streaming, Tasks running count, Git dirty + `↑/↓`. Surface **flat mặc định** (`liquidGlass=false`), glass opt-in. Home `/` là **bento dashboard** (`pages/index.vue`) thay redirect. Keep-alive `<NuxtPage :keepalive>` giữ nguyên. Xem [docs/features/ui-renew.md](../../../docs/features/ui-renew.md).
...
├── layouts/
│   └── default.vue         # Shell: NavRail (trái) | (AppTopBar + UpdateBanner + QuotaBanner + content slot) + SettingsModal
├── components/
│   │
│   │ # Primitive + layout (dùng chéo, giữ ở root) — auto-import dùng pathPrefix:false
│   ├── NavRail.vue, AppTopBar.vue   # shell mới (UI renew, ADR 0052)
│   ├── HeaderTabBar.vue             # shell cũ — giữ tạm để rollback, sẽ gỡ
