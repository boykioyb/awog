  ...
  <div class="fixed inset-0 z-50" @click="onBackdropClick">
    ...
    >
      ...
      >
        ...
        <div class="text-[0.86em] font-medium" :style="{ color: t.text }">Add MCP server</div>
...
const emit = defineEmits<{
  // Parent should hydrate the MCP list after close — the LLM may have written
  // a new config.json to disk during the conversation.
  close: []
}>()
...
const onSend = async () => {
  ...
  const messageId = `ma-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 6)}`
  currentMessageId.value = messageId

  const history = messages.value.slice(0, -1).map((m) => ({ role: m.role, text: m.text }))

  try {
    await sidecar.request('mcp.author', {
      messageId,
      history,
      userText: text,
    ...
    // Every turn may have written a new config.json — refresh the list now so
    // the user sees the server appear without having to close the modal.
    ws.hydrateMcpFromSidecar().catch(() => {
