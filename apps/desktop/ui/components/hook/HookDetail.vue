<template>
  <div class="p-4 md:p-6 max-w-3xl">
    ...
    <div class="flex flex-col sm:flex-row sm:items-start gap-3 mb-6">
      ...
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-2 mb-1 flex-wrap">
          ...
          <span
            class="text-[0.79em] px-1.5 py-0.5 rounded font-mono"
            :style="{
          ...
          <span class="text-[0.71em] px-1.5 py-0.5 rounded uppercase" :style="modeBadgeStyle">
            {{ hook.runMode }}
          </span>
        </div>
    ...
    <Section :title="`Recent runs · ${hook.recentRuns.length}`">
      <div v-if="hook.recentRuns.length === 0" class="text-[0.79em]" :style="{ color: t.textFaint }">
        Chưa chạy lần nào.
      </div>
      <div v-else class="space-y-1">
        <div
          ...
          :key="i"
          class="flex items-center gap-3 p-2 rounded text-[0.79em] flex-wrap"
