<template>
  <div v-if="step.kind === 'group'" class="text-[12px]">
    <button
    ...
    >
      ...
      <span
        ...
        class="inline-flex items-center justify-center text-[10px] px-1.5 rounded-sm font-mono"
        :style="{
  ...
  <div v-else-if="step.kind === 'thinking'" class="flex items-center gap-1.5 text-[12px]">
    <Brain :size="11" :style="{ color: t.textDim }" />
    <span class="italic" :style="{ color: t.textMuted }">{{ step.label }}</span>
  </div>

  <div v-else-if="step.kind === 'note'" class="text-[12px]" :style="{ color: t.text }">
    {{ step.label }}
  </div>
  ...
  >
    ...
    >
      ...
      <div class="flex-1 min-w-0">
        <div class="text-[12px] font-semibold flex items-center gap-1.5" :style="{ color: t.text }">
          Proposed plan
          <span
            class="text-[9px] px-1.5 py-0.5 rounded uppercase tracking-wider font-medium"
            :style="{
        ...
        <div v-if="step.label" class="text-[11px] mt-0.5" :style="{ color: t.textDim }">
          {{ step.label }}
        </div>
      </div>
    </div>
    <ol class="px-3 py-2 space-y-1 list-none">
      ...
      >
        <span
          class="inline-flex items-center justify-center w-5 h-5 rounded-full flex-shrink-0 text-[10px] font-semibold"
          :style="{
            background: planAccent.bg,
            color: planAccent.accent,
            border: `1px solid ${planAccent.border}`,
    ...
    <div
      ...
      class="px-3 py-2 text-[11px] italic"
    ...
    >
      <button
        class="inline-flex items-center gap-1 px-2.5 py-1 rounded text-[11px] font-medium transition"
      ...
      <button
        class="inline-flex items-center gap-1 px-2.5 py-1 rounded text-[11px] transition"
        :style="{
  ...
  <button
    v-else
    type="button"
    class="flex items-center gap-1.5 text-[12px] min-w-0 w-full text-left rounded py-0.5 px-1 -mx-1 transition"
    :style="{
  ...
  >
    ...
    <span
      v-if="step.target"
      class="inline-flex items-center px-1.5 rounded-sm font-mono text-[10px]"
      :style="{
        background: t.bgInput,
        color: t.text,
        border: `1px solid ${t.border}`,
    ...
    <span
      v-if="step.pathHint"
      class="font-mono text-[10px] truncate ml-auto"
