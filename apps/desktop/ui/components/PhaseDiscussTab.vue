<template>
  <div>
    <div class="text-[11px] mb-3 leading-relaxed" :style="{ color: t.textDim }">
    ...
    <div v-else class="space-y-2 mb-3">
      <div v-for="(msg, i) in run.messages" :key="i" class="flex gap-2">
        ...
        <div
          v-else
          class="w-5 h-5 rounded flex items-center justify-center text-[9px] font-semibold flex-shrink-0"
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-baseline gap-2 mb-0.5">
            <span class="text-[10px] font-medium" :style="{ color: t.text }">
              {{ msg.role === 'agent' ? agent.name : 'You' }}
            </span>
            <span class="text-[10px]" :style="{ color: t.textFaint }">{{ msg.at }}</span>
          </div>
    ...
    <div class="flex items-end gap-2">
      <textarea
        ...
        class="flex-1 rounded px-2 py-1.5 text-[11px] resize-none"
        :style="{
