  ...
  return {
    ...
    async execute(_id, params): Promise<AgentToolResult<WakeupDetails>> {
      if (armedThisTurn >= MAX_WAKEUPS_PER_TURN) {
        return errorResult(
          `You have already scheduled ${armedThisTurn} wake-ups this turn, which is the limit. Finish your answer to the user instead.`,
        )
      }
      try {
        const armed = await armWakeup({
          sessionId: input.sessionId,
          delaySeconds: params.in_seconds,
          note: params.note,
        })
        armedThisTurn += 1
        const clampNote =
