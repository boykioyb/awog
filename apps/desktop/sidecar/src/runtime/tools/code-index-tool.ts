...
const Params = Type.Object({
  ...
  path: Type.Optional(
    Type.String({
      description:
        'For blast: the file to change (repo-relative, absolute, or a unique path suffix like "sessions/runner.ts"). For define/refs: optional substring filter on the file path.',
    }),
  ),
  depth: Type.Optional(
    Type.Number({
      description: `For blast: how many import hops to follow (default ${DEFAULT_BLAST_DEPTH}, max ${MAX_BLAST_DEPTH}).`,
    }),
  ),
})
...
function fail(action: string, text: string): AgentToolResult<CodeIndexDetails> {
  return { content: [{ type: 'text', text }], details: { action, matches: 0, isError: true } }
}

function ok(action: string, matches: number, text: string): AgentToolResult<CodeIndexDetails> {
  return { content: [{ type: 'text', text }], details: { action, matches } }
}
