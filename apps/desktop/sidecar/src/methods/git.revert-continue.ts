// `git.revertContinue` — finish a conflicted revert (`git revert --continue`).
// Refuses cleanly when no revert is in progress so the UI can't fire it into
...
register('git.revertContinue', async (raw): Promise<{ ok: true }> => {
  const params = Params.parse(raw)
  if ((await pendingOpOf(params.workspaceRoot)) !== 'revert') {
    throw new RpcError(GIT_RPC_CODE, 'Không có revert đang chạy', {
  ...
  return withWorkspaceLock(params.workspaceRoot, async () => {
    ...
    try {
      // `core.editor=true`: reuse the existing message instead of hanging on an
      // interactive editor in a headless child.
      await runGit(params.workspaceRoot, ['-c', 'core.editor=true', 'revert', '--continue'])
    } catch (err) {
