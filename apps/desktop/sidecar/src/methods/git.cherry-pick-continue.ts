// `git.cherryPickContinue` — finish a conflicted cherry-pick (`git cherry-pick --continue`).
...
register('git.cherryPickContinue', async (raw): Promise<{ ok: true }> => {
  ...
  return withWorkspaceLock(params.workspaceRoot, async () => {
    ...
    try {
      ...
      await runGit(params.workspaceRoot, ['-c', 'core.editor=true', 'cherry-pick', '--continue'])
    } catch (err) {
