// `git.revertAbort` — restore the pre-revert state (`git revert --abort`).
...
register('git.revertAbort', async (raw): Promise<{ ok: true }> => {
  ...
  return withWorkspaceLock(params.workspaceRoot, async () => {
    ...
    try {
      await runGit(params.workspaceRoot, ['revert', '--abort'])
    } catch (err) {
