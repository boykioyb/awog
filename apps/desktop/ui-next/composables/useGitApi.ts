...
export interface SidecarGitTag {
  name: string
  sha: string
  sha7: string
  subject: string
...
export function useGitApi() {
  ...
  return {
    ...
    tagList: (workspaceRoot: string) =>
      sidecar.request<{ tags: SidecarGitTag[] }>('git.tagList', { workspaceRoot }),
    tagDelete: (workspaceRoot: string, name: string) =>
      sidecar.request<{ ok: true }>('git.tagDelete', { workspaceRoot, name }),
    ignore: (workspaceRoot: string, patterns: string[]) =>
      sidecar.request<{ ok: true }>('git.ignore', { workspaceRoot, patterns }),
    savePatch: (workspaceRoot: string, path?: string) =>
      sidecar.request<{ ok: true; path: string }>('git.savePatch', {
        workspaceRoot,
        ...(path !== undefined ? { path } : {}),
      }),
