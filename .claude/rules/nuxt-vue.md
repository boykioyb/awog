...
- **Font-size + line-height + radius: dùng CSS token, KHÔNG hardcode** ([ADR 0079](../../docs/decisions/0079-native-macos-shell-and-design-tokens.md)). `pnpm lint` chạy `scripts/check-design-tokens.mjs` và **fail** nếu vi phạm.
  ...
  - **Leading — 7 bậc:** `var(--lh-xs)` 16 · `var(--lh-sm)` 18 · `var(--lh-md)` 20 · `var(--lh-lg)` 22 · `var(--lh-xl)` 24 · `var(--lh-2xl)` 28 · `var(--lh-prose)` 22 (@base 13). **Cấm MỌI hệ số không đơn vị** — kể cả `1`, `2`, `0`. Hệ số thập phân nhân lại với font-size (`1.5` × 13px = 19.5px) nên trả nửa pixel về đúng chỗ type scale vừa khử; **hệ số nguyên** thì ghim hộp dòng ĐÚNG BẰNG font-size, mà 4/6 bậc `--fs-*` là LẺ ở base 13 (xs 11 · md 13 · lg 15 · xl 17) và mọi bậc lật chẵn/lẻ khi Appearance kéo base 12→18 ⇒ icon chẵn căn giữa lại rơi vào nửa pixel. Giá trị có đơn vị kế thừa thành **độ dài cố định**, không nhân lại. Hợp lệ: `var(--lh-*)` · **px CHẴN** · keyword CSS. Khai `font-size: var(--fs-X)` ⇒ khai luôn `line-height: var(--lh-X)`; `font-size` px cố định — hoặc hộp chật cố ý, tức chỗ trước đây viết `line-height: 1` — ⇒ **px chẵn**, làm tròn **LÊN** (không bao giờ thu chiều cao); văn bản dài dùng `--lh-prose`. Token khai bằng `round(up, calc(var(--font-size-base) * k), 2px)` nên hộp dòng **chẵn ở cả 6 base** — nguyên thôi chưa đủ, hộp dòng lẻ lại đẩy icon chẵn về nửa pixel.
  - **Hairline trên thanh cao cố định** dùng `box-shadow: inset 0 -1px 0 var(--border)`, KHÔNG `border-bottom` — border ăn 1px của content box (44 → 43) và đẩy con căn giữa vào nửa pixel. `inset` chứ không outset: outset bị background của sibling kế tiếp vẽ đè. Chỉ cho thanh **cao cố định + căn giữa con theo trục dọc**, không migrate toàn bộ border.
  ...
  - **Icon — 5 bậc, TẤT CẢ CHẴN, px cố định:** `var(--icon-xs)` 12 · `var(--icon-sm)` 14 · `var(--icon-md)` 16 (mặc định `.icn`) · `var(--icon-lg)` 20 · `var(--icon-xl)` 24. **Cấm cỡ icon px lẻ** — icon lẻ căn giữa trong hộp cao chẵn rơi vào nửa pixel (`(36 − 15) / 2 = 10.5`) nên nét vẽ không trúng lưới pixel. Lẻ thì map **lên**: 11→12, 13→14, 15→16, 17→18. Cỡ chẵn ngoài thang (10/18/22/26/28/40) hợp lệ. Icon **không** scale theo Appearance — nó là hình vẽ, không phải chữ.
  - `.icn` dùng `stroke-width: 1.5`. `stroke-width` là user unit của viewBox `0 0 24 24` ⇒ nét thật = `stroke-width × size/24`; ở 16px thì 1.5 ra đúng 1 device pixel. **`<Icon :size="13" />` là no-op** (`Icon.vue` chỉ có prop `name`) — đổi cỡ bằng `style` hoặc class.
