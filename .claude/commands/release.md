...
- Thêm vào **ĐẦU** mảng `CHANGELOG` ở CẢ HAI file (giữ đồng bộ y như các release trước):
  - @apps/desktop/ui-next/utils/changelog.ts
  - @apps/desktop/ui/utils/changelog.ts
...
- Commit: `docs(changelog): add vX.Y.Z release notes` (chỉ stage 2 file changelog.ts).

## 3. Bump version

Sửa field `"version"` từ giá trị cũ sang version mới ở **đúng 5 file** (mỗi file 1 dòng):
...
```bash
for f in package.json apps/desktop/electron/package.json apps/desktop/sidecar/package.json apps/desktop/ui-next/package.json apps/desktop/ui/package.json; do
  perl -i -pe 's/"version": "OLD"/"version": "NEW"/' "$f"
done
```
Commit: `chore(release): vX.Y.Z` (stage đúng 5 file đó).
