---
name: Nuxt and Vue conventions
description: Nuxt, Vue, Pinia and Tailwind conventions.
enabled: true
---
...
- **Font-size:**
  - **Body text dùng `text-[1em]`** (scale theo `--font-size-base` từ Appearance setting; default 12px, range `FONT_SIZE_MIN`=12 → `FONT_SIZE_MAX`=18 ở [composables/useAppearance.ts](../../apps/desktop/ui/composables/useAppearance.ts)). Không `text-xs`/`text-sm`/`text-[<1em]` cho text đọc được — để người dùng tự chỉnh cỡ qua setting thay vì hardcode nhỏ.
